# the inputs must be in the format of an array of string
...
matrix = Matrix.build(20000) { 0 } ;nil
x_pos = 0
y_pos = 0
distance = 0

input_wire_1.each_with_index do |move, step|
  direction = move[0]
  lenght = move[1..-1].to_i

  if direction == 'R' || direction == 'L'
    direction_multiplier = direction == 'R' ? 1 : -1
    lenght.times do
      x_pos += 1 * direction_multiplier
      distance += 1
      matrix[x_pos, y_pos] = distance
      puts "step #{step} x_pos #{x_pos} y_pos #{y_pos}"
    end
  end

  if direction == 'U' || direction == 'D'
    direction_multiplier = direction == 'U' ? 1 : -1
    lenght.times do
      y_pos += 1 * direction_multiplier
      distance += 1
      matrix[x_pos, y_pos] = distance
      puts "step #{step} x_pos #{x_pos} y_pos #{y_pos}"
    end
...
# find collision while going with wire 2
...
input_wire_2.each_with_index do |move, step|
  direction = move[0]
  lenght = move[1..-1].to_i
  ...
  if direction == 'R' || direction == 'L'
    ...
    lenght.times do
      ...
      if matrix[x_pos, y_pos] != 0
        collisions << [x_pos, y_pos, matrix[x_pos, y_pos], distance]
      end
      puts "step #{step} x_pos #{x_pos} y_pos #{y_pos}"
    end
  end
...
# find min manhattan distance, solving first question

collisions.map { |coordinates| coordinates[0].abs + coordinates[1].abs }.min

# find shorthest path sum collision, solving second question

collisions.map { |coordinates| coordinates[2] + coordinates[3] }.min
